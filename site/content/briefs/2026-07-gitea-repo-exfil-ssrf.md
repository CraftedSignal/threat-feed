...
mitre_ttps:
  ...
  - tactic_id: TA0004
    ...
    technique_id: T1556
    technique_name: Bypass Multi-Factor Authentication
    evidence: The 2FA bypass allows for multiple authenticated sessions from a single TOTP or stateless replay of a captured TOTP value for the full validity window.
    confidence_band: high
  - tactic_id: TA0003
    tactic_name: Persistence
    technique_id: T1136
    technique_name: Create Account
    evidence: An attacker can mint a personal access token via POST /api/v1/users/{username}/tokens inside that window for long-lived access that outlives the captured OTP.
    confidence_band: high
cves:
  - id: CVE-2026-25038
    cvss: 7.5
...
updates:
  ...
  - at: "2026-07-21T21:50:25Z"
    ...
    source_urls:
      - https://github.com/advisories/GHSA-v73x-hx65-6pf4
  - at: "2026-07-21T21:58:24Z"
    level: L2
    summary: 'merged source coverage: Gitea Vulnerable to TOTP 2FA Bypass via Race Condition and API Replay'
    sources:
      - ghsa
    source_urls:
      - https://github.com/advisories/GHSA-gx3v-q759-g323
